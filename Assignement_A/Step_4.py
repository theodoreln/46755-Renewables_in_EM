...
def Multiple_hour_optimization(Generators, Wind_Farms, Demands, Nodes) :
    # Global variables
    ...
    for t in range(n_hour):
        for n in range(1,25):
            ...
            model.addConstr(gp.quicksum(var_dem[d-1,t] for d in D) 
                ...
                + gp.quicksum(L[j][1] * (var_theta[n-1,t] - var_theta[L[j][0]-1]) for j in range(len(L)-1)) == 0)
    ...
    if model.status == GRB.OPTIMAL:
        ...
        optimal_theta = [[round(var_theta[n,t].X,2) for e in range(1,25)] for t in range(n_hour)] #syntax?
    ...
    return(optimal_conv_gen, optimal_wf_gen, optimal_dem, optimal_elec, optimal_theta)
...
def Select_one_hour(Generators, Demands, optimal_conv_gen, optimal_wf_gen, optimal_dem, optimal_elec,optimal_theta, select_hour) :
...
def Copper_plate_multi_hour(Generators, Wind_Farms, Demands) :
    optimal_conv_gen, optimal_wf_gen, optimal_dem, optimal_elec = Multiple_hour_optimization(Generators, Wind_Farms, Demands, Nodes)
